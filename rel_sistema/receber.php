...
<body>	
	...
	<div class="titulo_cab titulo_img"><u>Recebimentos <?php echo $tipo_rel ?> <?php echo $acao_rel ?> <?php echo $texto_pessoa ?>  </u></div>	
...
$vencidas = 0;
		...
		if($pessoa != ""){
			$query = $pdo->query("SELECT * FROM receber where $busca >= '$dataInicial' and $busca <= '$dataFinal' and pago LIKE '$pago' and pessoa = '$pessoa' and tipo LIKE '%$tipo%' and empresa = '$id_empresa' ORDER BY id desc");
		}else{
			$query = $pdo->query("SELECT * FROM receber where $busca >= '$dataInicial' and $busca <= '$dataFinal' and pago LIKE '$pago' and  tipo LIKE '%$tipo%' and empresa = '$id_empresa' ORDER BY id desc");
		}
