...
if($pessoa != ''){
	$texto_pessoa = ' Cliente: '.$nome_pessoa;	
}else{
	$texto_pessoa = '';	
}
...
<body>	
	...
	<div class="titulo_cab titulo_img"><u>Relatório de Vendas <?php echo $acao_rel ?> <?php echo $texto_pessoa ?>  </u></div>	
...
$vencidas = 0;
		...
		if($pessoa != ""){
			$query = $pdo->query("SELECT * FROM receber where $busca >= '$dataInicial' and $busca <= '$dataFinal' and pago LIKE '$pago' and pessoa = '$pessoa' and tipo = 'Venda' and empresa = '$id_empresa' ORDER BY id desc");
		}else{
			$query = $pdo->query("SELECT * FROM receber where $busca >= '$dataInicial' and $busca <= '$dataFinal' and pago LIKE '$pago' and  tipo = 'Venda' and empresa = '$id_empresa' ORDER BY id desc");
		}
...
<td><img src="<?php echo $url_sistema ?>img/<?php echo $imagem ?>" width="10px" height="10px" style="margin-top: 4px"> <?php echo $data_lancF ?></td> 
					<td class="esc">R$ <?php echo $valorF ?></td>	
				<td class="esc"><?php echo $data_vencF ?></td>				
				<td class="esc"><?php echo $nome_pessoa ?></td>
				<td class="esc"><?php echo $saida ?></td>
...
exit();
	...
	<div class="col-md-12 p-2" style="margin-top: 20px">
		...
		<span class="text-success"> <small><small><small><small>RECEBIDAS</small> : R$ <?php echo @$recebidasF ?></small></small></small>  </span>
