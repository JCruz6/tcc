...
if($vencidas != ""){
	$query = $pdo->query("SELECT * FROM $tabela where data_venc < curDate() and pago != 'Sim' and empresa = '$id_empresa' order by data_venc asc");
}else{
...
<td><i class="fa fa-square {$classe_pago} mr-1"></i> {$descricao}</td> 
				...
				<td>
					<big><a class="{$ocultar}" href="#" onclick="editar('{$id}', '{$descricao}', '{$pessoa}','{$valor}','{$data_venc}','{$frequencia}','{$tumb_arquivo}', '{$tipo}')" title="Editar Dados"><i class="fa fa-edit text-primary "></i></a></big>

					<big><a href="#" onclick="mostrar('{$id}', '{$descricao}', '{$nome_pessoa}','{$valor}','{$data_lancF}','{$data_vencF}','{$data_pgtoF}','{$nome_usu_lanc}','{$nome_usu_pgto}','{$nome_frequencia}','{$tumb_arquivo}','{$pago}','{$arquivo}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
